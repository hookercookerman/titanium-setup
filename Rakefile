...
def compile_sass
  ...
  compilation = system "sass --compass -C -t expand app/hello_ti/stylesheets/app.sass > Resources/app.jss"
end

def compile_coffee
  puts "Compiling CoffeeScript".blue
  paths = `find app/hello_ti -name '*.coffee'`.split("\n")
  compilation = (
  system "coffee -c -b --join Resources/hello_ti.js -c -b #{paths.join(' ')}" and
